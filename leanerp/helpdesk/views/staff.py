...
from lfmarket.roles import ISSUE_TRACKING
...
if helpdesk_settings.HELPDESK_ALLOW_NON_STAFF_TICKET_UPDATE:
    ...
    staff_member_required = user_passes_test(
        ...
        login_url="/user/access_violation.html",
...
def get_user_tickets_count(request):
...
def dashboard(request):
    ...
    """
    print("========================")
    print(request.user.has_perm("helpdesk.view_ERP_issues"))
    print("========================")

    if not request.user.has_perm("helpdesk.view_ERP_issues"):
...
def create_ticket(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # create Notification
            if Notification.objects.filter(unread=1, 
                                           recipient=request.user,
                                           verb='有新的工作指派給你').count() == 0:
                notify.send(sender=request.user, 
                            recipient=request.user, 
                            verb='有新的工作指派給你',
                            href='/user/helpdesk/dashboard/')
            # end of createw Notification
